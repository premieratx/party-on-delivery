-- NUCLEAR SECURITY LOCKDOWN - Eliminate ALL Public Data Access (FIXED)
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND information_schema.tables.table_name = current_table) THEN
            ...
            RAISE NOTICE 'Dropped all policies from table: %', current_table;
        END IF;
    END LOOP;
END $$;
...
BEGIN
    ...
    LOOP
        IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND information_schema.tables.table_name = current_table) THEN
            ...
            EXECUTE format('REVOKE ALL ON TABLE public.%I FROM PUBLIC', current_table);
            -- Revoke from anon role  
            EXECUTE format('REVOKE ALL ON TABLE public.%I FROM anon', current_table);
            -- Revoke from authenticated role
            EXECUTE format('REVOKE ALL ON TABLE public.%I FROM authenticated', current_table);
            
            RAISE NOTICE 'Revoked all privileges from table: %', current_table;
        END IF;
    END LOOP;
END $$;
...
-- Handle orders table if it exists
DO $$
BEGIN
...
-- Handle quotes table if it exists
DO $$
BEGIN
