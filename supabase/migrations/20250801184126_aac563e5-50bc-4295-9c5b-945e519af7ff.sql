...
-- Create function to safely store recent order data
CREATE OR REPLACE FUNCTION public.store_recent_order(
  p_session_id TEXT,
  ...
  p_is_group_order BOOLEAN DEFAULT false
)
RETURNS UUID
LANGUAGE plpgsql
SECURITY DEFINER
...
DECLARE
  result_id UUID;
BEGIN
  INSERT INTO public.recent_orders (
    session_id,
    payment_intent_id,
    share_token,
    order_number,
  ...
  ON CONFLICT (session_id) DO UPDATE SET
    ...
    is_group_order = EXCLUDED.is_group_order,
    created_at = now()
