-- FINAL SECURITY LOCKDOWN - FIXED COLUMN AMBIGUITY
-- Resolves column reference ambiguity and implements absolute security
...
BEGIN
    FOREACH tbl_name IN ARRAY all_sensitive_tables LOOP
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE tables.table_name = tbl_name AND tables.table_schema = 'public') THEN
            EXECUTE format('REVOKE ALL ON public.%I FROM PUBLIC, anon', tbl_name);
...
BEGIN
    FOREACH tbl_name IN ARRAY all_tables LOOP
        IF EXISTS (SELECT 1 FROM information_schema.tables WHERE tables.table_name = tbl_name AND tables.table_schema = 'public') THEN
            ...
            RAISE NOTICE 'Forced RLS on: %', tbl_name;
        END IF;
    END LOOP;
END $$;

-- 3. DROP ALL EXISTING POLICIES ON SENSITIVE TABLES
DO $$ 
DECLARE
...
-- SECURE OTHER TABLES IF THEY EXIST
DO $$
BEGIN
    -- AFFILIATE ORDER TRACKING
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'affiliate_order_tracking' AND t.table_schema = 'public') THEN
    ...
    -- ORDERS TABLE
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'orders' AND t.table_schema = 'public') THEN
    ...
    -- QUOTES TABLE
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'quotes' AND t.table_schema = 'public') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'affiliate_referrals' AND t.table_schema = 'public') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'commission_payouts' AND t.table_schema = 'public') THEN
    ...
    IF EXISTS (SELECT 1 FROM information_schema.tables t WHERE t.table_name = 'telegram_users' AND t.table_schema = 'public') THEN
...
VALUES (
    'comprehensive_security_lockdown_final',
    'system',
    jsonb_build_object(
        'action', 'maximum_security_enforcement_applied',
        'revoked_public_access', true,
        'forced_rls_enabled', true,
        'strict_policies_created', true,
        'timestamp', now(),
