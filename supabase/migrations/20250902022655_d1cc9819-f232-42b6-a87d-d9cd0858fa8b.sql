-- Enhanced trigger function that handles deduplication
...
BEGIN
  ...
  IF NEW.status IN ('completed', 'confirmed', 'delivered') AND 
    ...
    -- Remove any abandoned order with the same session_id to prevent duplicates
    DELETE FROM abandoned_orders WHERE session_id = NEW.session_id;
    
    -- Trigger the sync to Google Sheets
