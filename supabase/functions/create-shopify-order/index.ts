...
${discountCode ? `🏷️ AFFILIATE TRACKING: Discount code "${discountCode}" used for affiliate sales tracking.` : ''}`
    ...
    if (!isJoiningGroupOrder) {
      // Create new customer order record with share token
      try {
        const { data: customerOrder, error: customerOrderError } = await supabaseClient
          ...
          .insert({
            ...
            shopify_order_id: orderResult.order.id.toString(),
            status: 'confirmed',
            share_token: shareToken, // This was generated by gen_random_uuid() in the database
          })
          .select('share_token')
          .single();

        if (!customerOrderError && customerOrder) {
          shareToken = customerOrder.share_token;
          logStep('Customer order record created', { shareToken });
        } else {
